...
bool UART::ComPort_Set(unsigned int BaudRate, unsigned int Parity, unsigned int ByteSize, unsigned int StopBits)
{
	...
	*************************************/
	...
	//options.c_cflag |= (BaudRate | Parity | ByteSize | StopBits | CLOCAL | CREAD);	
	...
	options.c_lflag &= ~(ICANON | ECHO | ECHOE | ISIG); // 使用非正規
	
	tcflush(COM_Handle, TCIFLUSH);

	// 特殊控制字元 c_cc[NCCS]，c_cc 陣列成員用來定義支援的特殊控制字元，及一些timeout參數。
	//options.c_cc[VMIN] = 0;
	//options.c_cc[VTIME] = 1; // 單位(0.1秒)(value: unsigned char 0~255)，設定 timeout 時間0.1秒
